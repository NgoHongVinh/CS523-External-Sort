...
function assign_values_to_file_records(num_of_records_per_page, num_of_records_of_file) {
    ...
    if (Array.isArray(INPUT_VALUES) && INPUT_VALUES.length > 0) {
        ...
        // clone để tránh mutate mảng gốc
        values = INPUT_VALUES.slice(0, num_of_records_of_file);

        // nếu file ít hơn số record cần → pad thêm
        while (values.length < num_of_records_of_file) {
            values.push(0);
        }

    } else {

        console.warn("INPUT_VALUES rỗng → fallback random");

        // fallback random nếu chưa load file
        for (let i = 0; i < num_of_records_of_file; i++) {
            values.push(parseFloat((Math.random() * 1000).toFixed(3)));
        }
    }
...
function refresh_file_record_values(values, num_of_records_per_page) {
    ...
    // ===== chuẩn hoá min/max để scale màu =====
    ...
    $('#file .record').each(function (index, record) {
        ...
        let display = Number(value).toFixed(3);

        if (num_of_records_per_page <= 6) {
            $(record).text(display);

        } else if (num_of_records_per_page <= 10) {
            $(record).append('<div class="record-value">' + display + '</div>');

        } else {
            $(record).append('<div class="record-value hidden-value">' + display + '</div>');
        }

        let color = calculate_record_color_css_from_value(value, min, max);
        $(record).css('background-color', color);

    });

    // ===== sort theo float chuẩn =====
    original_records_values_order.sort((a, b) => a.value - b.value);
}

function calculate_record_color_css_from_value(value, min, max) {

    // normalize 0..1
    let normalized = (value - min) / (max - min);
    normalized = Math.max(0, Math.min(1, normalized));

    // nén phân bố màu để giảm độ lệch chuẩn
    normalized = Math.sqrt(normalized);

    // dải màu: xanh lá → teal → xanh lam nhẹ
    const start_hue = 130;   // xanh lá
    const end_hue   = 200;   // teal/xanh lam nhẹ

    const hue = start_hue + normalized * (end_hue - start_hue);

    // saturation vừa phải để không chói
    const saturation = 55;

    // lightness cao hơn chút để UI sáng, rõ
    const lightness = 60;
...
function display_phase_description_and_code_line(phase_info, display_for_forward_step) {
	...
	if (selected_animation_speed_multiplier < get_animation_speed_multiplier("speed_1_50") && animation_is_playing && current_phase < animation.length - 3) {
		description_to_display = "<span class=\"text-info-note\">LƯU Ý</span> Hãy đặt tốc độ mô phỏng về 1.5x hoặc thấp hơn, hoặc sử dụng <b>phím mũi tên trái/phải</b> hoặc <b>nút lùi/tiến</b> để xem <b>mô tả từng bước</b> của thuật toán.";
	}
...
function display_phase_pass_and_io_ops(pass, phase_io_operations, display_for_forward_step) {
	...
	let current_pass_html = "<span>Lần duyệt:</span><span><b>i=" + pass + "</b></span>"
	if (pass == -2) current_pass_html = "<span>Hoàn thành</span>"	//  End of algorithm
	if (pass == -1) current_pass_html = "<span>Khởi động...</span>"	// Start of algorithm
	...
	let io_ops_html = "<span>Số lượng I/O:</span><span><b>" + input_output_operations_counter + "</b></span>"
...
function reset_displayed_phase_pass_and_io_ops() {
	// Reset current pass
	let current_pass_html = "<span style='font-size: 0.7rem; letter-spacing: -0.035vw; color: white;'>Nhấn <span class='play-pause-icon button-icon' style='display: inline-block; margin-top: -1vw; transform: translateY(0.27vw)'></span> hay <span class='step-forward-icon button-icon' style='margin-top: -0.1vw; transform: translateY(0.14vw)'></span> để bắt đầu</span>" // Start of algorithm
	let current_pass_elem = $("#current-pass");
	current_pass_elem.html(current_pass_html);

	// Reset I/O operations counter
	let io_ops_elem = $("#io-operations");
	let io_ops_html = "<span style='font-size: 0.7rem; letter-spacing: -0.035vw; color: white;'>Nhấn <span class='sorting-tree-icon button-icon' style='display: inline-block; margin-top: -1vw; transform: translateY(0.27vw)'></span>  để xem cây sắp xếp</span></span>"
	io_ops_elem.html(io_ops_html);
...
function restart(show_confirmation_if_playing = true, custom_confirm_message = "") {
	...
	if (confirmed) {
		...
		if (num_of_records_of_file == -1) num_of_records_of_file = INPUT_VALUES.length;
		if (num_of_pages_of_file == -1) num_of_pages_of_file = Math.ceil(INPUT_VALUES.length/num_of_records_per_page);
		...
		<div id="input-values">
			...
			<div class="input-group">
				<div>Số lượng Buffer:</div>
				<div><b>M</b> = <input type="number" id="frames-in-buffer" value="${num_of_frames_in_buffer}" min="1"></div>
			</div>


			<div class="input-group">
				<div>Số lượng phần tử tối đa mỗi trang:</div>
		...
		if (selected_algorithm_type == null) selected_algorithm_type = 1;
		time = 0;
...
function add_button_functions() {
	...
	// force click on the "k-way" button on start (to set it as selected algorithm)
	selected_algorithm_type = null;
	$("#algorithm-selector #k-way").click();
	...
	// Add functions to the toggle buttons #focus-toggle, #code-highlight-toggle
	...
	$("#sorting-tree-button").click(function () {
		...
		let string = "Cây sắp xếp của tệp với B=" + num_of_pages_of_file + " trang, " + num_of_records_of_file + " số phần tử, tối đa " + num_of_records_per_page + " phần tử mỗi trang và M=" + num_of_frames_in_buffer + " trang buffer khả dụng trong buffer.<br/>Mỗi dòng hiển thị tệp và các lần chạy đã được sắp xếp ở cuối mỗi lần truyền của thuật toán " + "K-way External Sorting" + ".";
...
function extractFloatsFromText(text) {
...
// biến global lưu dữ liệu từ file
let INPUT_VALUES = [];
